...
async function ensureProgramDeployed(
  ...
  // Solana 1.18's bundled Cargo cannot read lockfile v4 generated by
  // the current GitHub runner. Generate it with the host Cargo, then rewrite
  // only the lockfile format marker to v3 before cargo-build-sbf reads it.
  run('cargo', ['generate-lockfile'], programDir);
  const cargoLockPath = path.join(programDir, 'Cargo.lock');
  if (fs.existsSync(cargoLockPath)) {
    const lock = fs
      .readFileSync(cargoLockPath, 'utf8')
      .replace(/^version = 4$/m, 'version = 3');
